...
def _route_budget_score(estimated_cost: float, budget: float) -> float:
    ...
    if estimated_cost <= budget:
        return _clamp(1 - (estimated_cost / budget) * 0.25)
    over_ratio = (estimated_cost - budget) / budget
    return _clamp(0.65 - over_ratio * 0.75)
...
def _route_reason_codes(score_detail: dict[str, Any], preferences: list[str]) -> list[str]:
    ...
    if score_detail["estimated_cost"] <= score_detail["budget"]:
